# Mistral perks — what this platform could offer

Wishlist for what a conformant Mistral consumer SHOULD offer
to manifests that declare `platforms.mistral`. Items land as the
vendor ships them.
...
- **Connector prefill** — `agent.connectors[]` pre-populates the Agents-console connector picker.
- **JSON-mode preview** — consoles that support JSON mode preview the validated output shape from `response_format`.
