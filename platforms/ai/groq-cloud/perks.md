# GroqCloud perks — what this platform could offer

Wishlist for what a conformant GroqCloud consumer SHOULD offer
to manifests that declare `platforms.groq-cloud`. Items land as the
vendor ships them.
...
- **Catalogue auto-resolve** — consumers pick the closest catalogue id when `model` matches a family rather than an exact id.
