# Replicate perks — what this platform could offer

Wishlist for what a conformant Replicate consumer SHOULD offer
to manifests that declare `platforms.replicate`. Items land as the
vendor ships them.
...
- **Hardware auto-pick** — consumers match `hardware` against the model's supported classes and downgrade with a warning if the class is unavailable.
