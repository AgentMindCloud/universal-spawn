# Anthropic MCP perks — what this platform could offer

Wishlist for what a conformant Anthropic MCP consumer SHOULD offer
to manifests that declare `platforms.anthropic-mcp`. Items land as the
vendor ships them.
...
- **Host-compat badge** — consumers verify `hosts[]` at registration time and mark the server as compatible with each listed host in the registry card.
