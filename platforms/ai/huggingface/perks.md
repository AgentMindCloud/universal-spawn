# Hugging Face perks — what this platform could offer

Wishlist for what a conformant Hugging Face consumer SHOULD offer
to manifests that declare `platforms.huggingface`. Items land as the
vendor ships them.
...
- **Space hardware auto-pick** — when `runtime.gpu_required` is true and `hardware` is unset, consumers suggest `t4-small` or better.
