# CrewAI perks — what this platform could offer

Wishlist for what a conformant CrewAI consumer SHOULD offer
to manifests that declare `platforms.crewai`. Items land as the
vendor ships them.
...
- **Crew validator** — consoles validate that every `tasks[].role` resolves to a `roles[].name` before the first spawn.
