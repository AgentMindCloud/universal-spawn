# Netlify perks — what this platform could offer

Wishlist for what a conformant Netlify consumer SHOULD offer
to manifests that declare `platforms.netlify`. Items land as the
vendor ships them.
...
- **Deploy context gating** — `env_contexts[]` controls which contexts a required secret must exist in before a build proceeds.
