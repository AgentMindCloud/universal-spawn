# Google Cloud (GCP) perks — what this platform could offer

Wishlist for what a conformant Google Cloud (GCP) consumer SHOULD offer
to manifests that declare `platforms.gcp`. Items land as the
vendor ships them.
...
- **IAM minimisation** — `safety.min_permissions` narrows the service-account IAM bindings to the smallest envelope.
- **Region allowlist** — `safety.data_residency` constrains the allowed GCP regions.
