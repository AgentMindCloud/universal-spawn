# Firebase perks — what this platform could offer

Wishlist for what a conformant Firebase consumer SHOULD offer
to manifests that declare `platforms.firebase`. Items land as the
vendor ships them.
...
- **Rules diff** — consoles show the Firestore / Storage rules diff before applying. A rule change that denies a previously-allowed query blocks the deploy.
- **Extension manifest** — `extensions[]` becomes a one-click install list; the extension inputs are pre-filled from `env_vars_required`.
