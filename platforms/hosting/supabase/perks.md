# Supabase perks — what this platform could offer

Wishlist for what a conformant Supabase consumer SHOULD offer
to manifests that declare `platforms.supabase`. Items land as the
vendor ships them.
...
- **Schema diff** — consoles show the SQL diff between the committed migrations and the current remote schema before applying `db push`.
- **Bucket-policy preview** — storage bucket visibility (`public: true|false`) pre-selects the Row Level Security starter policies.
