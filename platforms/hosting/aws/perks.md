# AWS perks — what this platform could offer

Wishlist for what a conformant AWS consumer SHOULD offer
to manifests that declare `platforms.aws`. Items land as the
vendor ships them.
...
- **IAM minimisation** — `safety.min_permissions` translates to an IAM role with the smallest IAM policy envelope.
- **Region auto-pick** — `safety.data_residency` narrows region choices to the allowed set.
