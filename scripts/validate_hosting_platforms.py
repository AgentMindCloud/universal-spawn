#!/usr/bin/env python3
"""Validate every platform folder under platforms/hosting/**.
...
For each folder that contains a `*-spawn.schema.json`:
  ...
  2. For each example under `examples/`, validate against the
     platform schema (with the master v1.0 schema registered).
...
def main(argv: list[str]) -> int:
    ...
    for schema_path in sorted((REPO / "platforms/hosting").rglob("*-spawn.schema.json")):
    ...
    print(
        f"\n{total_platforms} hosting platform(s), {total_examples} "
        f"example(s), {failed} failure(s)"
    )
